...
- **Celery** — система фоновых задач, которая выполняет операцию отправки письма с подтверждением регистрации) **асинхронно**, вне цикла HTTP-запроса.  
- **Redis** — брокер сообщений, который хранит задачи в очередях и передаёт их Celery worker’ам для обработки.  
...
![Примерное содержимое файла .env](MemoSpace/file env.png)
